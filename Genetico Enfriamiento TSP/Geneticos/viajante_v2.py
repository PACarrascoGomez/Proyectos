...
for i in range(n_generaciones):
	...
	if modo_verbose:
		...
		for j in indices_padres:
			print "Padre: ",j
	# Cruce: Cruce por corte de un punto, sin repetir nodos (ciudades)
	hijos_cruzados = []
	conjunto_nodos = set(nodos)
	for j in range(len(indices_padres)):
		if j == len(indices_padres)-1: # Cruce ultimo nodo con el primero
			padre1 = poblacion[indices_padres[j]]
			padre2 = poblacion[indices_padres[0]]
		else:
			padre1 = poblacion[indices_padres[j]]
			padre2 = poblacion[indices_padres[j+1]]
		particion = int(round((len(padre1)-1)/2.0))
		hijo = padre1[0:particion]
		hijo += padre2[particion:len(padre2)-1]
		conjunto_hijo = set(hijo)
		nodos_a_insertar = list(conjunto_nodos - conjunto_hijo)
		nodos_visitados = []
		for z in range(len(hijo)):
			if hijo[z] in nodos_visitados:
				hijo[z] = nodos_a_insertar.pop()
			else:
				nodos_visitados.append(hijo[z])
		hijo.append(hijo[0]) # nodo inicial = nodo final
