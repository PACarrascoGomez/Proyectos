...
if len(sys.argv) != 3:
	print ("Uso: python3 experimentacion5.py <fichero_xml> <fichero_out>")
	sys.exit(1)
...
g = GetSynsets()
resultados = {}
for k in distancias_coseno_frases:
    pun = puns[k]
    # Obtenemos el sentido1
    s1 = sentido_PYWSD(pun,frases[k])
    if s1[1]:
        s1 = s1[0].lemmas()[0].key()
        # Obtenemos el sentido2
        distancias_coseno = distancias_coseno_frases[k]
        distancias_coseno_ord = sorted(distancias_coseno)
        p1 = distancias_coseno_ord[0][1]
        s2, s3 = g.select_best_different(pun, p1, p1)
        if s2 != None and s1 != s2:
            resultados[k] = (s1,s2)
