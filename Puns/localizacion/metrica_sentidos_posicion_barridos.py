...
# Metodo: Distancia coseno + ponderacion posicion
...
if len(sys.argv) != 3:
	print ("Uso: python3 metrica_sentidos_posicion_barridos.py <f_xml> <f_gold>")
	sys.exit(1)
...
#frases_tok = tokenizar_frases(frases_test,stopwords) # Analisis sobre test
	...
	for i in range(len(palabras)):
		for j in range(i+1,len(palabras)):
			...
			if p1 != p2 and not(contiguas(p1,p2,frases[k])):
				...
				d_min = ponderar_distancia(p2,d_min,palabras_frase) # Variar wp y wd en la funcion ponderar_distancia()
				if d_min != float("Inf"):
