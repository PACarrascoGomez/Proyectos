...
# Trabajo: Extra (Test con Ngramas) -> trigramas
###################################################################
...
# y se han generado los corpus con 3gramas.py
# Separamos el corpus de train en dos partes (train+dev)
# 85% -> train; 15% -> dev
n_frases_train=$(cat train_3gramas.en | wc -l)
n_frases_dev=$(echo "("$n_frases_train*"15)/100" | bc)
n_frases_train=$(echo $n_frases_train"-"$n_frases_dev | bc)
cat train_3gramas.en | head -$n_frases_train > $train.en
cat train_3gramas.es | head -$n_frases_train > $train.es
cat train_3gramas.en | tail -$n_frases_dev > $dev.en
cat train_3gramas.es | tail -$n_frases_dev > $dev.es
